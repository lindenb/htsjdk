...
public class BamFileIoUtils {
    ...
    public static void gatherWithBlockCopying(final List<File> bams, final File output, final boolean createIndex, final boolean createMd5) {
        try {
            ...
            // It is possible that the modified time on the index file is ever so slightly more recent than the original BAM file
            ...
            if (createIndex && (output.lastModified() < indexFile.lastModified())) {
                ...
                if (!success) {
                    System.err.print(String.format("Index file is newer than BAM file for %s and unable to resolve this", output.getAbsolutePath()));
                }
            }
        }
        catch (final IOException ioe) {
